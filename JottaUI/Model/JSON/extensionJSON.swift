...
extension DecodeJSON: Comparable {
    public static func == (lhs: DecodeJSON, rhs: DecodeJSON) -> Bool {
        ...
        case let (.array(left), .array(right)): left as NSArray == right as NSArray
        case let (.dictionary(left), .dictionary(right)): left as NSDictionary == right as NSDictionary
        case (.null, .null): true
        default: false
        }
    }
...
public extension NSNumber {
    ...
    var jsonIsBool: Bool {
        ...
        let isObjCBool = String(cString: objCType) == "c"
