    ...
    var body: some View {
            ...
            ZStack {
                if let logfilerecords {
                    Table(logfilerecords) {
                        ...
                        TableColumn(
                            "Logfile Jotta-client" + ": \(logfilerecords.count) rows" + sortdirectionstring()
                        ) { data in
        ...
        .onChange(of: sortdirection) {
            Task {
                ...
                if filterstring.isEmpty == false {
                    // Must read all logrecords before filter
                    let alllogrecords = await ActorJottaCliLogfile().sortjottaclilogfile(direction: sortdirection)
                    logfilerecords = alllogrecords.filter { $0.logrecordline.contains(filterstring) }
                } else {
                    logfilerecords = await ActorJottaCliLogfile().sortjottaclilogfile(direction: sortdirection)
                }
                updateactionlogview = false
            }
        }
